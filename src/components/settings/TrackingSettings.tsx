  ...
  return (
    <div className="bg-theme-panel p-6 rounded-xl shadow-sm border border-theme-item">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-bold text-theme-text-secondary mb-1">Google Analytics ID</label>
          <input
            type="text"
            value={config.gaId || ''}
            onChange={(e) => handleIdChange('gaId', e.target.value)}
            className="w-full p-2 text-sm border rounded-lg bg-theme-item border-theme-item text-theme-text outline-none transition-colors"
            placeholder="e.g. G-12345ABCDE"
          />
        </div>
        
        <div className="pt-2 border-t border-theme-grid-divider mt-2">
          <label className="block text-xs font-bold text-theme-text-secondary mb-1 mt-2">Umami Website ID</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-xs font-bold text-theme-text-secondary mb-1">Self-Hosted Umami Script URL (Optional)</label>
          <input
            type="url"
          ...
          />
          <p className="text-[10px] text-theme-text-secondary mt-1 font-medium">
